			...
			<div id="panel" class="radius">
				...
				<div id="content" class="clearfix">
					...
					<table cellpadding="1" cellspacing="1" id="resultTable">
						<thead>
							<tr>
								<th  style="border-left: 1px solid #C1DAD7"> Mã Xác Nhận </th>
								<th> Tên </th>
								<th> Địa chỉ </th>
								<th> Liên hệ </th>
								<th> Email </th>
						...
						<?php
							include('connect.php');
							$result = mysqli_query($link,"SELECT * FROM reservation ORDER BY firstname ASC");
							while($row = mysqli_fetch_array($result))
								{
									...
									$results = mysqli_query($link,"SELECT * FROM rooinventory where confirmation='$rrr'");
									while($row1 = mysqli_fetch_array($results))
										{
										$roomid=$row1['room'];
										$resulta = mysqli_query($link,"SELECT * FROM internet_shop where id='$roomid'");
